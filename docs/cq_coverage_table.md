...
Counts come from running the current query set against the latest validated completed KG from `20260423T003657Z`, where all `20/20` queries returned at least one row and `16` of `17` validation rules passed. The single failing rule (V15) has warning severity only and `0` errors.
...
| --- | --- | ---: | --- |
| CQ01 | Answered | 12 | Parliamentary debates are linked to policy topics and dates, mainly through PMQs-style records. |
| CQ02 | Answered | 55 | Ministerial statements are linked to issuing government departments through official-source records. |
| CQ03 | Answered (soft) | 8 | Political-actor extraction is useful but still depends on partial actor disambiguation. |
| CQ04 | Answered | 155 | Confirms events can be both reported by news articles and represented in official source records. |
| CQ05 | Answered | 7 | Shows shared policy topics across parliamentary and government policy events. |
| CQ06 | Answered | 12 | Parliamentary-body links are populated consistently; the explicit parent-type assertion fix ensures ParliamentaryDebate instances satisfy the ParliamentaryEvent type check in the query. |
...
| CQ09 | Diagnostic | 28 | Intentionally identifies reported events still missing a government body, parliamentary body, or topic; the count reflects the smaller dataset in this run producing proportionally more events without complete institution grounding. |
| CQ10 | Answered | 16 | Policy-topic frequency query over reported parliamentary and government policy events. |
| CQ11 | Answered | 150 | Cross-source question requiring both official-source representation and a department link; the completed graph supports this strongly with 13 additional rows over the prototype. |
...
| CQ13 | Answered | 37 | Topic co-occurrence query exercises event-topic self-joins. |
| CQ14 | Answered | 16 | Comparative parliamentary-versus-government topic coverage query. |
| CQ15 | Answered | 8 | Location aggregation over events with populated occursInLocation links. |
...
- CQ11 shows the strongest completion benefit with 13 additional rows over the prototype.
