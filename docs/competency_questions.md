...
| CQ09 | `PolicyEvent`, `OfficialBody`, `GovernmentBody`, `ParliamentaryBody`, `PolicyTopic` | missing-link detection over `involvesGovernmentBody`, `occursInParliamentaryBody`, `concernsPolicyTopic` | Completion-oriented CQ over the relevant subclasses of `OfficialBody`; explicitly supports targeted graph-audit and coverage analysis. |
...
| CQ19 | `ParliamentaryEvent`, `MinisterialStatement`, `PolicyTopic` | `occursOnDate`, `concernsPolicyTopic` | Temporal-topic overlap across disjoint event subclasses; stronger than date-only co-occurrence and well aligned to the current event-centred KG. |
